...
hxltm_extras_action_bin="$1"
# hxltm_extras_action_infile="${2:-fontem.ext}"
hxltm_extras_action_infile="${2}"
# hxltm_extras_action_outfile="${3:-objectivum.ext}"
hxltm_extras_action_outfile="${3}"
hxltm_extras_action_args="$4"

# if [ -n "$working_languages_str" ]; then
#     hxltm_extras_action_args="$hxltm_extras_action_args --agendum-linguam $working_languages_str"
# fi

# if [ -n "$non_working_languages_str" ]; then
#     hxltm_extras_action_args="$hxltm_extras_action_args --non-agendum-linguam $non_working_languages_str"
# fi

# if [ -n "$auxiliary_languages_str" ]; then
#     hxltm_extras_action_args="$hxltm_extras_action_args --auxilium-linguam $auxiliary_languages_str"
# fi

# if [ -n "$source_language" ]; then
#     hxltm_extras_action_args="$hxltm_extras_action_args --fontem-linguam $source_language"
# fi

# if [ -n "$target_language" ]; then
#     hxltm_extras_action_args="$hxltm_extras_action_args --objectivum-linguam $target_language"
# fi

# if [ -n "$export_ad_hoc_template" ]; then
#     hxltm_extras_action_args="$hxltm_extras_action_args --objectivum-formulam $export_ad_hoc_template"
# fi

# if [ -n "$export_data_exchange_standard" ]; then
#     hxltm_extras_action_args="$hxltm_extras_action_args --objectivum-$export_data_exchange_standard"
# fi

# if [ -n "$dump_abstract_syntax_tree" ]; then
#     hxltm_extras_action_args="$hxltm_extras_action_args --expertum-HXLTM-ASA $dump_abstract_syntax_tree"
# fi

# TODO: debug

# if echo "$hxltm_extras_action_bin" | grep -q "hxl"
# then
#     # TODO: simple check for spaces, and etc
#     echo "INFO: [$hxltm_extras_action_bin] Okay"
# else
#     echo "ERROR: [$hxltm_extras_action_bin] not hxltml or libhxl-python binary"
#     exit 1
# fi

hxltm_extras_action_cmd="$hxltm_extras_action_bin $hxltm_extras_action_args $hxltm_extras_action_infile $hxltm_extras_action_outfile"

# echo "::group::source file"
# ls -lha "$hxltm_extras_action_infile"
# echo "::endgroup::"

echo "::group::hxltm_extras_action_cmd"
echo "$hxltm_extras_action_cmd"
echo "::endgroup::"

echo "::group::resultatum"
resultatum="$($hxltm_extras_action_cmd)"
echo "$resultatum"
echo "::endgroup::"
